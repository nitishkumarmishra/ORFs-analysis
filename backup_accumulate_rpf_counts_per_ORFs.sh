...
perl -pe 's/\r\n|\n|\r/\n/g' $ORFPREF.orfs.bed >$TMPWF.orfs.bed
sort  -t $'\t'  -k1,1  -k2,2n  -k3,3n  -o $TMPWF.orfs.bed  $ORFPREF.orfs.bed
...
bsub -n 8 -J rna -e sort.err -o sort.out -q priority -P RSEM -R 'rusage[mem=25000]' "sort -t $'\t' -k1,1 -k2,2n --parallel=8 -o $TMPWF.orfs.classchr.bed  $TMPWF.orfs.classchr.bed"
...
samtools  sort -n $TMPWF.bam -o $TMPWF.bam
samtools  view -hb $TMPWF.bam -o $TMPWF.rsem.map.bam
