...
## This part is for the heatmap plot ny using ComplexHeatmap tool
...
ht11=Heatmap(df_score, col=col_score, cluster_rows=cluster_rows, 
             ...
             rect_gp=rect_gp, 
             heatmap_legend_param=list(title=type_col, title_gp=gpar(fontsize=8), labels_gp=gpar(fontsize=9)),
             row_title=row_title,
...
get_heatmap_row_annotation <- function(df, vec_color_group, nv_genes=NULL,
  ...
  top_annotation <- HeatmapAnnotation(df=df_annot,col=list(group=col_group), 
                                      annotation_legend_param=list(group=list(title=column_annot_title, title_gp=column_annot_title_gp, 
                                                                              labels_gp=column_annot_labels_gp, grid_height=unit(3,"mm"), ncol=column_annot_ncol)))
  ...
  ha12 <- rowAnnotation(df=df_annot, col=list(group=col_group),
                        annotation_legend_param=list( group=list(title="gene set", title_gp=gpar(fontsize=8), labels_gp=gpar(fontsize=5))),
